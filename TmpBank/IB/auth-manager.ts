...
namespace AuthManagerControl {

    interface IDigitalSignatureEvents {
        // every request related event starts with onRequest[event name]
        onRequestSucceeded(data: any);
        pnRequestFailed(data: any); // when user rejects signing or when our request times out
        onRequestBeforeStart(); // this event fires right before the request to hamoon/sign
        onRequestTrackingDataReceived(data: any); // when response of hamoon/sign is retrieved
        onRequestStatusCheck(data: any); // when response of request to hamoon/inquiry is retrieved
    }
    ...
    export class ActionsToEfects {
        ...
        constructor(private _digitalSigManager: AuthManagerControl.AuthManager) {
        }
    ...
    export class AuthManager {
        private static DIGITAL_SIG_SUBMIT_EVENT_NAME = "AUTHMANAGERDigitalSigSubmitClickedEvent";

        private _wrapper: HTMLDivElement;
        private _submitBtn: HTMLInputElement;
        private _allAuthMethodsWrapper: HTMLElement;
        ...
        private _authManagerControlId: string;
        ...
        private static _wrapperIdToDigSigManagerInstance: { [key: string]: AuthManager } = {};

        private _isDigSigMethodSelected = false;
        private _hasInitialized = false;

        // events
        private _onAuthMethodMethodChangedEvent: (selectedAuthMethodInput: HTMLInputElement, lastSelectedAuthMethodInput: HTMLInputElement) => void; // when auth method changes, for instance changing dig-sig-method to username-pass-method
        private _digitalSignatureEvents: IDigitalSignatureEvents;
        ...
        private _defaultDigitalSignatureInitRequestData: DigitalSigRequestData = {
            endpoint: () => `${AuthManager._baseApiUrl}/digitalsig/init`,
            data: () => { return {} }
        };

        private _defaultDigitalSignatureCheckStatusRequestData: DigitalSigRequestData = {
            endpoint: () => `${AuthManager._baseApiUrl}/digitalsig/check`,
            data: () => { return {} }

        };

        public static createInstance(
            authManagerControlId: string,
            wrapperId: string,
            interval: number,
            action: Action,
            ...
            AuthManager._wrapperIdToDigSigManagerInstance[wrapperId] = instance;
            instance._authManagerControlId = authManagerControlId;
        ...
        public static getInstance(wrapperID: string): AuthManager {
            const instance = AuthManager._wrapperIdToDigSigManagerInstance[wrapperID];
            return instance;
        }
        ...
        private _startDigitalSigProcess() {
            this._digitalSignatureEvents && typeof this._digitalSignatureEvents.onRequestBeforeStart === 'function' && this._digitalSignatureEvents.onRequestBeforeStart();
            $.ajax({
                ...
                success: (response) => {
                    ...
                    this._digitalSignatureEvents && typeof this._digitalSignatureEvents.onRequestTrackingDataReceived === 'function' && this._digitalSignatureEvents.onRequestTrackingDataReceived(response);
                    this._checkRequestStatus();
                },
                error: (errors: any) => {
                    this.enableWrapper();
                    this._digitalSignatureEvents && typeof this._digitalSignatureEvents.pnRequestFailed === 'function' && this._digitalSignatureEvents.pnRequestFailed(errors);
                },
                xhrFields: {
        ...
        private _setAuthMethodsRadioBtnListeners() {
            ...
            this._onAutheMethodChangedHanlder(this._targetAuthMethodRb, null, this.getSelectedAuthMethod());

            this._allAuthMethodsWrapper.addEventListener("change", (e) => { this._onAutheMethodChangedHanlder(this._targetAuthMethodRb, e) });
        }

        // for fieldset/div[class has AuthMethodsWrapperClass] html change event
        private _onAutheMethodChangedHanlder(digSigRb: HTMLInputElement, e?: Event, selectedElement?: HTMLInputElement) {
            ...
            if (element instanceof HTMLInputElement && (element as HTMLInputElement).type === "radio") {
                ...
                e && this._onAuthMethodMethodChangedEvent && typeof this._onAuthMethodMethodChangedEvent === 'function'
                    && this._onAuthMethodMethodChangedEvent(element, this._lastSelectedAuthMethodRb);
            ...
            $.ajax({
                ...
                success: (response: any) => {
                    if (response.Status == DigitalSignatureStatus.SUCCEEDED) {
                        this._digitalSignatureEvents && typeof this._digitalSignatureEvents.onRequestSucceeded === 'function' && this._digitalSignatureEvents.onRequestSucceeded(response);
                        //here
                    } else if (response.Status == DigitalSignatureStatus.FAILED || response.Status == DigitalSignatureStatus.TIMED_OUT) {
                        this.enableWrapper();
                        this._digitalSignatureEvents && typeof this._digitalSignatureEvents.pnRequestFailed === 'function' && this._digitalSignatureEvents.pnRequestFailed(response);
                    } else {
                        setTimeout(() => { this._checkRequestStatus(); }, this._interval);
                        this._digitalSignatureEvents && typeof this._digitalSignatureEvents.onRequestStatusCheck === 'function' && this._digitalSignatureEvents.onRequestStatusCheck(response.Status)
                    }
                },
                error: (errors: any) => {
                    this.enableWrapper();
        ...
        public setDigitalSignatureListeners(digEventsImpl: IDigitalSignatureEvents) {
            this._digitalSignatureEvents = digEventsImpl;
        }
        ...
        public setOnAuthMethodChangedListener(listener: (selectedElement, lastElement) => void) {
        ...
        public performPostBack() {
            try {
                // @ts-ignore
                // listen to DigSig.Submit event in server side
                performPostBack(this._wrapper.querySelector(`#${this._authManagerControlId}`), { selectedAuthMethodName: this.getSelectedAuthMethod().name });
            } catch (e) {
        ...
        public static isFirstInstance() {
            return Object.keys(AuthManager._wrapperIdToDigSigManagerInstance).length == 0;
        }

        public getSelectedAuthMethod(): HTMLInputElement {
            return Array.prototype.some.call((this._allAuthMethodsWrapper.querySelectorAll("input[type=radio]") as NodeListOf<HTMLInputElement>), (element) => {
                if (element.checked) {
                    return true;
                }
                return false;
